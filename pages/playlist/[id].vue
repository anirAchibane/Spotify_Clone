<template>
    ...
    <div v-if="playlist" class="playlist-container">
        <div class="playlist-header">            <img :src="playlist.images && playlist.images.length > 0 ? playlist.images[0]?.url : '/default-playlist.png'" 
                 alt="Playlist Cover" 
                 class="playlist-cover" />
            <div class="playlist-info">                <span class="playlist-type">PLAYLIST</span>
                ...
                <div v-if="isEditing" class="edit-form">
                    ...
                    <div class="edit-buttons">
                        <button @click="savePlaylistChanges" class="save-button" :disabled="saving">
                            {{ saving ? 'Saving...' : 'Save' }}
                        </button>
                ...
                <!-- Image Upload Modal -->
                <div v-if="showImageUploadModal" class="modal-overlay" @click.self="showImageUploadModal = false">
                    <div class="modal-content">
                        <h3>Change Playlist Image</h3>
                        <div class="upload-area" @click="triggerFileInput" @dragover.prevent @drop="onFileDrop">
                            <input 
                                type="file" 
                                ref="fileInput" 
                        ...
                        <div class="modal-buttons">
                            ...
                            <button @click="showImageUploadModal = false" class="cancel-button">
                                Cancel
                            </button>
                        </div>
                    </div>
        ...
        <div class="playlist-content">
            ...
            <div class="tracks-list">                <div v-for="(item, index) in playlist.tracks.items" 
                     :key="item.track?.id || `track-${index}`"
                     class="track-item"
                     @click="item.track?.id && navigateToTrack(item.track.id)">
                    <div class="track-number">{{ index + 1 }}</div>
                    <div class="track-title">                        <img :src="item.track.album.images && item.track.album.images.length > 0 
                             ? (item.track.album.images[2]?.url || item.track.album.images[0]?.url) 
                             : '/default-track.png'" 
                             :alt="item.track.name"
                             class="track-image">
                        <div class="track-info">
                            <div class="track-name">{{ item.track.name }}</div>                            <div class="track-artists">
                    ...
                    </div>                    <div class="track-album" @click.stop="item.track.album?.id && navigateToAlbum(item.track.album.id)">
...
const savePlaylistChanges = async () => {
    ...
    try {
        ...
        isEditing.value = false;
        showNotification('Playlist details updated successfully!', 'success');
    } catch (error) {
        console.error('Error updating playlist:', error);
        showNotification(`Failed to update playlist: ${error.message}`, 'error');
    } finally {
        saving.value = false;
    }
};
...
const handleSelectedFile = (file) => {
    if (file.size > 4 * 1024 * 1024) { // 4MB limit for initial upload
    ...
    reader.onload = (e) => {
        ...
        if (file.size > 200 * 1024) { // If larger than ~200KB, resize
    ...
    img.onload = () => {
        ...
        fetch(resizedDataUrl)
            ...
            .then(blob => {
                ...
                console.log(`Original size: ${Math.round(selectedFile.value?.size / 1024)}KB, Resized: ${Math.round(resizedFile.size / 1024)}KB`);
            ...
            .catch(err => {
                ...
                showNotification('Could not process image. Please try another one.', 'error');
            });
    };
...
const uploadImage = async () => {
    ...
    try {
        ...
        console.log(`Estimated base64 image size: ${Math.round(estimatedSize / 1024)}KB`);
        ...
        showNotification('Playlist image updated successfully!', 'success');    } catch (error) {
        ...
        if (error.message.includes('NetworkError') || error.message.includes('Failed to fetch')) {
            ...
            showNotification('Image is too large. Please use a smaller image or let us resize it for you.', 'error');
        } else {
...
onMounted(async () => {
  const token = localStorage.getItem("access_token");
  if (!token) {
    showNotification('Not logged in. Please log in to view playlists.', 'error');
    router.push('/');
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 404) {
        showNotification('Playlist not found or you don\'t have permission to view it.', 'error');
        router.push('/myPlaylists');
        return;
      }
    ...
    if (playlist.value && playlist.value.owner && currentUserId.value) {
      isUserPlaylistOwner.value = playlist.value.owner.id === currentUserId.value;
      console.log(`User is ${isUserPlaylistOwner.value ? '' : 'not '}the playlist owner`);
    }
  } catch (err) {
    console.error("Error fetching playlist:", err);
    showNotification(`Error loading playlist: ${err.message}`, 'error');
  }
});
</script>
...
.track-artists, .track-album, .track-date-added, .track-duration {
    color: #b3b3b3;
}
...
.photo-button:hover, .delete-button:hover {
    border-color: white;
}
